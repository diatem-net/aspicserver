...
class Config {
    ...
    /**
     * Récupère une valeur de configuration. (localeConfig dans le fichier config)
     * @param string $key   Clé de la valeur à récupérer
     */
    public static function getLocalConfig($key){
        if(isset(self::$data['localConfig']) &&
                isset(self::$data['localConfig'][$key])){
            return self::$data['localConfig'][$key];
        }else{
            throw new \Exception('La configuration '.$key.' n\'existe pas dans localConfig');
        }
    }
